...
(function (a, b, c) {
	...
	b.Mason.settings = {isResizable: !0, isAnimated: !1, animationOptions: {queue: !1, duration: 500}, gutterWidth: 0, isRTL: !1, isFitWidth: !1}, b.Mason.prototype = {_getBricks: function (a) {
		var b = this.options.itemSelector, c = b ? a.filter(b).add(a.find(b)) : a;
		c.css({position: "absolute"}).addClass("masonry-brick");
		return c
	}, _create: function (c) {
		...
		this.element.prepend(i), this.offset.y = Math.round(i.position().top), this.options.isRTL ? this.offset.x = Math.round(i.position().left) : (i.css({"float": "right", display: "inline-block"}), this.offset.x = Math.round(this.element.outerWidth() - i.position().left)), i.remove();
		var j = this;
		setTimeout(function () {
		...
		if (b.isPlainObject(a))this.options = b.extend(!0, this.options, a); else {
		...
		for (var j = 0, k = a.length; j < k; j++) {
			...
			if (e === 1)this._placeBrick(d, this.cols, this.colYs); else {
				f = this.cols + 1 - e, g = [];
		...
		var l = {};
		l.height = Math.max.apply(Math, this.colYs) - this.offset.y, this.options.isFitWidth && (l.width = this.cols * this.columnWidth - this.options.gutterWidth), this.styleQueue.push({$el: this.element, style: l});
		...
		var d = Math.min.apply(Math, c), e = d + a.outerHeight(!0), f = c.length, g = f, h = this.cols + 1 - f, i = {};
		while (f--)c[f] === d && (g = f);
		i.top = d, i[this.horizontalDirection] = this.columnWidth * g + this.offset.x, this.styleQueue.push({$el: a, style: i});
		...
		var b = this.find("img"), d = b.length, e = "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==", f = this;
		if (!d) {
			a.call(this);
			return this
		}
